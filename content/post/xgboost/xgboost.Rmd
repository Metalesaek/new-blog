...
Decision tree^[Kevin P.Murphy 2012] is a model that recursively splits the input space into regions and defines local model for each resulted region. However, fitting decision tree model to complex data would not yield to accurate prediction in most cases, which can be termed as [weak learner](http://rob.schapire.net/papers/strengthofweak.pdf). But combining multiple decision trees togather (called also **ensemble models**) using techniques such as aggregating  and boosting can largely improve the model accuracy. [Xgboost](https://xgboost.readthedocs.io/en/latest/R-package/index.html) (short for Extrem gradient boosting) model is a tree-based algorithm that uses these types of techiques. It can be used for both **classification** and **regresesion**. 
In this paper we learn how to implement this model to predict the well known titanic data as we did in th previous papers using different kind of models. 
...
Let's take a look at this data using the function glimpse 
...
The featues are defined as follows:
...
As We see, twe have 177 missing values from age variable and 2 values from Embarked. For missing values we have two strageties, removing completely the missing values from the analysis, but doing so we will lose many data,  or imputing them by one of the available imputation method to fix these values. Since we have large number of missing values compared to the total examples in the data it would be better to follow the latter strategi. Thankfully to [mice](https://cran.r-project.org/web/packages/mice/mice.pdf) package that is a very powrfull for this purpose and it provides many imputation methods for all variable types. 
We will opt for random forest method since in most cases can be the best choice. However, in order ot respect the most importante rule in machine learning, never touch the test data during the training process , we will apply this imputation after spliting the data.   
...
The left side of the plot shows that higher fraction of females survived, whreas the right side shows the reverse situation for males where most of them died. We can induce from this plot that, ceteris paribus, this predictor is likely to be relevant for prediction.        
...
in this plot most of the first class passengers survived in contrast with the third class passegers where most of them died. However, for the second class, it seems equally ballanced. Again this predictor also can be relevant. 
...
This predictor refers to the number of siblings a passenger has. It seems to be equally distibuted given the taregt variable, and hence can be highlly irrelvant. In other words, knowing the number of siblings of a particular passenger does  not help to predict if this passenger survived or died. 
...
This predictor refers to the number of parents and children a passenger has. It seems that this predictor  is slightly dsicriminative if we look closely at the level 0, passengeers with no parents or children. 
...
We see that a passsenger who is embarked from the port **S** is slightly highly to died, while the other ports seem to be eequally distributed.
...
We see that some signicant overlaping between the two conditional distribution may indicating less relevance related to this variable.
...
we can also plot two predictors against eeach other. For instance let's try with the two predictors, Sex and Pclass:
...
Note that the -1 value added to the formula is to avoid adding a column as intercept with ones to our data. we can take a look at the structure of the data by the follwoing
...
We know that many machine learning algorithms require the inputs to be in a specific type. The input types supported by xgboost algorithm are: matrix, **dgCMatrix** object rendred from the above package **Matrix**, or the xgboost class **xgb.DMatrix**.  
...
* gamma (default=0): also called min_split_loss, the minimum loss required for spliting further a particular node.
...
* subsample (default=1): with the default the model uses all the data at each tree, if 0.7 for instance, then the model randomly sample 70% og the data at each iteration, doing so we fight the overfiting problem.
...
To evaluate the model we will use the test data that should follow all the above steps as the training data exept for the missing values. since the test set is only used to evaluate the model so we will remove all the missing values.
...
Then we use the predit function and and confusionMatrix function from caret package, and since the predicted values are prbabilities we convert them to predicted classes using the threshold of 0.5 as follows: 
...
with the default values we obtain a pretty good acuraccy rate. The next step we fine tune the hypeparameters sing **cross validation** with the help of caret package.   
...
Now instead we use the **train** function from caret to train the model and we specif the mothod as **xgbtree**.
...
**Note**: This model took several minutes so we do not the model to be rerun again when rendring this document that is why i have commented the above script and have saved the results in csv file, then i reload it again to continue our analysis. If you would like to run this model you can just uncomment the script.  
...
Now we apply these values for the final model using the whole data uploadded at the begining from the train.csv file, and then we call the file test.csv file for titanic data to submmit our prediction to the kaggle competition.    
...
Additionally, when inspecting the sammary above we notice that we have an extra level (9) in the factor **Parch** that is not existed in the traind data, and hence the model does not allow such extra information.However, since this level has only two cases we can approximate this level by the closest one which is 6, the we drorp the level 9 from this fator.
...
As we see the uotput is a probability of each instance, so we should convert this probabilitis to classe labels:
...
Finally, we save the file as csv file in submmit it to kaggle then check our rank :
...
Xgboost is the most machine learning algorithm in nowdays due to its powerfull capability to predict wide range of data from various domaine. Several win competitions in kaggle and elswhere are achieved by this model. Using its capability to train in parallel fashion, it can handle large and complex data with ease. the large number of Hyperparameters that has give the modeler a large possibilities to tune the model with respect to the data at their hand as well as to fight other problems such as overfitting, feature selection...ect.
